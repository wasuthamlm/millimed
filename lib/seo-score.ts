...
function scoreAeo(input: SeoScoreInput): SeoCategoryResult {
  ...
  return category([
    ...
    toCheck("มีรายการ (bullet) หรือขั้นตอนในเนื้อหา", "ช่วยให้ AI ดึงคำตอบเป็นลิสต์ได้ง่ายขึ้น", hasList ? 25 : 0, 25),
    toCheck(
...
export function pageToScoreInput(page: PageSeoAdapterInput): SeoScoreInput {
  const bodyText = page.sections.map((s) => [s.titleTh, s.bodyTh].filter(Boolean).join(" ")).join("\n\n");
  return {
