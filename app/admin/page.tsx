...
import { Product, Post, ContactMessage } from "@/lib/db/models/index";
...
export default async function AdminDashboardPage() {
  const [session, totalProducts, totalArticles, publishedArticles, pendingApproval, unreadMessages, latestArticles] =
    await Promise.all([
      auth(),
      Product.count(),
      Post.count(),
      ...
      Post.findAll({ limit: 5, order: [["updatedAt", "DESC"]] }),
    ]);

  return (
    <div className="flex flex-col gap-8">
      <PageHeader icon={GridIcon} title="Dashboard" subtitle={`ยินดีต้อนรับ, ${session?.user?.email ?? ""}`} />

      <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard icon={BoxIcon} label="สินค้าทั้งหมด" value={totalProducts} caption="ทุก status" />
        <StatCard
          icon={FileTextIcon}
          label="บทความทั้งหมด"
          value={totalArticles}
        ...
        <StatCard
          ...
          tone="gold"
        />
        <StatCard icon={MailIcon} label="ข้อความที่ยังไม่อ่าน" value={unreadMessages} caption="จากผู้เยี่ยมชม" />
      </div>
            ...
            <tbody>
              {latestArticles.map((post) => (
                <tr key={post.id} className="border-b border-slate-50 last:border-0">
                  <td className="max-w-xs truncate px-6 py-3.5 font-medium text-slate-800">{post.titleTh}</td>
