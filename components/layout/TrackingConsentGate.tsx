...
// reload the document. Ported from the Millimed BFS site's
// components/layout/TrackingConsentGate.tsx, which itself adapted the legacy
// millimedbfs.com source app's TrackingManager.jsx (minus its per-route "marketing
// eligibility" opt-out system — no equivalent concept exists in this project either).
...
export function TrackingConsentGate({
  ...
  useEffect(() => {
    ...
    if (window.__millimedMetaPixelLoaded && typeof window.fbq === "function") window.fbq("track", "PageView");
    if (window.__millimedTikTokPixelLoaded && window.ttq?.page) window.ttq.page();
