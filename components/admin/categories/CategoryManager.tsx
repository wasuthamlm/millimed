...
export function CategoryManager({
  ...
  const ordered = hasHierarchy ? topLevel.flatMap((parent) => [parent, ...childrenOf(parent.id)]) : categories;
  ...
  const submitEdit = (id: string) => {
    ...
    startTransition(async () => {
      ...
      setCategories((prev) =>
        prev.map((c) =>
          c.id === id ? { ...c, nameTh: editForm.nameTh, nameEn: editForm.nameEn || null, slug: editForm.slug, parentId: editForm.parentId } : c
        )
      );
      setEditingId(null);
    });
  ...
  const reorderWithinGroup = (draggedId: string, targetId: string, groupParentId: string | null) => {
    ...
    setCategories((prev) => {
      ...
      return hasHierarchy && groupParentId === null ? [...reorderedGroup, ...others] : [...others, ...reorderedGroup];
    });
      ...
      {adding && (
        <div className="flex flex-col gap-3 rounded-2xl border border-slate-100 bg-white p-4 shadow-sm sm:flex-row sm:items-end sm:flex-wrap">
          <div className="flex-1 min-w-[160px]">
            ...
            <input className={inputClass} value={addForm.nameTh} onChange={(e) => setAddForm((f) => ({ ...f, nameTh: e.target.value }))} />
          </div>
          <div className="flex-1 min-w-[160px]">
            <label className="mb-1 block text-xs text-slate-500">ชื่ออังกฤษ</label>
            <input className={inputClass} value={addForm.nameEn} onChange={(e) => setAddForm((f) => ({ ...f, nameEn: e.target.value }))} />
          </div>
          <div className="flex-1 min-w-[160px]">
            <label className="mb-1 block text-xs text-slate-500">Slug (เว้นว่างให้สร้างอัตโนมัติ)</label>
            <input className={inputClass} value={addForm.slug} onChange={(e) => setAddForm((f) => ({ ...f, slug: e.target.value }))} />
          </div>
          {hasHierarchy && (
            <div className="flex-1 min-w-[160px]">
            ...
            {ordered.map((row) => {
              ...
              return (
                <tr
                  ...
                  className={cn("border-b border-slate-50 last:border-0", dragId === row.id && "opacity-50")}
                >
                  {isEditing ? (
                    <>
                      <td className="px-4 py-3 text-slate-300">{groupIndex}</td>
                      <td className="px-2 py-3">
                        <input className={inputClass} value={editForm.nameTh} onChange={(e) => setEditForm((f) => ({ ...f, nameTh: e.target.value }))} />
                      </td>
                      <td className="px-6 py-3">
                        <input className={inputClass} value={editForm.nameEn} onChange={(e) => setEditForm((f) => ({ ...f, nameEn: e.target.value }))} />
                      </td>
                      <td className="px-6 py-3">
                      ...
                      <td className="px-6 py-3">
                        <div className="flex items-center gap-2">
                          <button type="button" disabled={pending} onClick={() => submitEdit(row.id)} className="rounded-md px-2 py-1 text-xs font-medium text-brand-navy hover:bg-slate-100">
                            บันทึก
                          </button>
                          <button type="button" onClick={() => setEditingId(null)} className="rounded-md px-2 py-1 text-xs text-slate-500 hover:bg-slate-100">
                            ยกเลิก
                          </button>
                        </div>
                      </td>
                    ...
                    <>
                      <td className="px-4 py-3 text-slate-400">
                        <div className="flex items-center gap-1.5">
                          {onReorder && <GripIcon className="h-4 w-4 shrink-0 cursor-grab text-slate-300" />}
                          {groupIndex}
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-3.5">
                        <div className="flex items-center justify-end gap-1">
                          ...
                          <button type="button" onClick={() => startEdit(row)} className="rounded-md p-1.5 text-slate-500 hover:bg-slate-100 hover:text-brand-navy" aria-label="แก้ไข">
                            <PencilIcon className="h-4 w-4" />
                          </button>
                          <button type="button" onClick={() => remove(row.id)} className="rounded-md p-1.5 text-red-500 hover:bg-red-50" aria-label="ลบ">
