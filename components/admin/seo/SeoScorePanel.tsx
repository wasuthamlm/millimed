...
function CategoryChecklist({ title, subtitle, result }: { title: string; subtitle: string; result: SeoCategoryResult }) {
...
export function SeoScorePanel({ result }: { result: SeoAeoGeoResult }) {
  ...
  return (
    <div className="flex flex-col gap-4 rounded-2xl border border-slate-100 bg-white p-6 shadow-sm">
      ...
      {tab === "geo" && <CategoryChecklist title="GEO — ถูกอ้างอิงโดย AI" subtitle="เกณฑ์ประมาณการ" result={result.geo} />}
    </div>
  );
}
