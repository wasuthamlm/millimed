...
function findActiveParent(pathname: string | null) {
  return adminNavItems.find((item) => item.children?.some((child) => isChildActive(pathname, child.href)))?.href;
}
    ...
    <div className="min-h-screen bg-slate-50">
                              ...
                              <li key={child.href}>
                                <Link
                                  ...
                                  className={cn(
                                    ...
                                    active ? "bg-white font-semibold text-brand-navy" : "text-white/70 hover:bg-white/10 hover:text-white"
                                  )}
                                >
                                  {child.label}
                                </Link>
      ...
      >
        <div className="sticky top-0 z-20 flex items-center gap-3 border-b border-slate-200 bg-white px-4 py-3 lg:hidden">
          ...
          <span className="text-sm font-semibold text-brand-navy">มิลลิเมด Admin</span>
        </div>
