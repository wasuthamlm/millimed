...
import { createPost, deletePost, updatePost, type PostFormInput } from "@/app/admin/articles/actions";
...
export function PostForm({ initialPost, categories = [] }: { initialPost?: InitialPost; categories?: ArticleCategoryOption[] }) {
  ...
  const seoResult = useMemo(
    ...
    [form.titleTh, form.titleEn, form.seoTitle, form.seoTitleEn, form.seoDesc, form.seoDescEn, form.excerptTh, form.bodyTh, form.slug, form.coverImageUrl]
  );
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-1 gap-4 rounded-2xl border border-slate-100 bg-white p-6 shadow-sm sm:grid-cols-2">
        ...
        <div>
          ...
          <select className={inputClass} value={form.status} onChange={(e) => update("status", e.target.value as InitialPost["status"])}>
        ...
        <div className="sm:col-span-2">
          ...
          <textarea className={inputClass} rows={8} value={form.bodyTh} onChange={(e) => update("bodyTh", e.target.value)} />
        </div>

        <div className="sm:col-span-2">
          <label className={labelClass}>เนื้อหา (อังกฤษ)</label>
          <textarea className={inputClass} rows={8} value={form.bodyEn} onChange={(e) => update("bodyEn", e.target.value)} />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 rounded-2xl border border-slate-100 bg-white p-6 shadow-sm sm:grid-cols-2">
        ...
        <div>
          ...
          <input className={inputClass} value={form.seoTitleEn} onChange={(e) => update("seoTitleEn", e.target.value)} maxLength={70} />
        </div>
        <div>
        ...
        <div>
          ...
          <textarea className={inputClass} rows={3} value={form.seoDescEn} onChange={(e) => update("seoDescEn", e.target.value)} maxLength={200} />
        </div>
        ...
        <div className="sm:col-span-2 flex flex-col gap-2 border-t border-slate-100 pt-4">
          ...
          <label className="flex items-center gap-2 text-sm text-slate-700">
            <input type="checkbox" checked={form.seoNoIndex} onChange={(e) => update("seoNoIndex", e.target.checked)} />
        ...
        <div className="sm:col-span-2 border-t border-slate-100 pt-4">
          ...
          <p className="truncate text-sm text-slate-600">{`${SITE_URL}/${canonicalPath}/${form.slug || "…"}`}</p>
        </div>
      </div>
