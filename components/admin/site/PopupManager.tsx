    ...
    <div className="flex flex-col gap-6">
      <div className="rounded-2xl border border-slate-100 bg-white p-5 shadow-sm">
        <div className="mb-5 flex items-center justify-between">
          ...
          <Toggle checked={config.enabled} onChange={(v) => setConfig({ ...config, enabled: v })} label="เปิดใช้งาน Popup" />
        </div>
        ...
        <div className="grid gap-5 sm:grid-cols-[200px_1fr]">
          ...
          <div className="flex flex-col gap-4">
            <div>
              ...
              <input type="text" value={config.titleTh} onChange={(e) => setConfig({ ...config, titleTh: e.target.value })} className="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm outline-none focus:border-brand-navy" />
            </div>
            <div>
              <label className="mb-1.5 block text-xs font-medium text-slate-500">ลิงก์ปลายทาง</label>
              <input type="text" value={config.link} onChange={(e) => setConfig({ ...config, link: e.target.value })} className="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm font-mono text-slate-500 outline-none focus:border-brand-navy" />
            </div>
          </div>
        </div>
        ...
        <div className="mt-5 grid gap-4 sm:grid-cols-3">
          ...
          <div>
            ...
            <input type="date" value={config.startDate} onChange={(e) => setConfig({ ...config, startDate: e.target.value })} className="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm outline-none focus:border-brand-navy" />
          </div>
          <div>
            <label className="mb-1.5 block text-xs font-medium text-slate-500">วันที่สิ้นสุด</label>
            <input type="date" value={config.endDate} onChange={(e) => setConfig({ ...config, endDate: e.target.value })} className="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm outline-none focus:border-brand-navy" />
          </div>
        </div>
      </div>
