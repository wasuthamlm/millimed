...
export function HeaderAppearanceForm({ initial, navLinks }: { initial: HeaderConfigInput; navLinks: NavLink[] }) {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-col gap-4 rounded-2xl border border-slate-100 bg-white p-6 shadow-sm">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <div>
            ...
            <select className={inputClass} value={form.menuLevels} onChange={(e) => update("menuLevels", Number(e.target.value))}>
          ...
          <div>
            ...
            <select className={inputClass} value={form.submenuStyle} onChange={(e) => update("submenuStyle", e.target.value)}>
            ...
            {SUBMENU_CHILD_OPTIONS.map((opt) => (
              <label
                ...
                className={cn(
                  ...
                  form.submenuChildBehavior === opt.value ? "border-brand-navy bg-brand-navy/5" : "border-slate-200 hover:bg-slate-50"
                )}
              >
                <input
                  type="radio"
      ...
      <div className="flex flex-col gap-4 rounded-2xl border border-slate-100 bg-white p-6 shadow-sm">
        ...
        <p className="-mt-2 text-xs text-slate-400">หมายเหตุ: Search / Account / Cart เป็นสวิตช์เตรียมไว้สำหรับเมื่อฟีเจอร์เหล่านั้นถูกสร้างขึ้นจริงในเว็บไซต์</p>
        <div className="grid gap-3 sm:grid-cols-2">
          {(
            ...
            <label key={item.key} className="flex cursor-pointer items-center gap-3 rounded-lg border border-slate-200 px-4 py-3">
              <input type="checkbox" checked={form[item.key]} onChange={(e) => update(item.key, e.target.checked)} className="h-4 w-4 accent-brand-navy" />
      ...
      <div className="flex flex-col gap-3 rounded-2xl border border-slate-100 bg-white p-6 shadow-sm">
        <div className="flex items-center justify-between">
          ...
          <div className="flex overflow-hidden rounded-lg border border-slate-200">
            <button
              ...
              className={cn("px-3 py-1.5 text-xs font-medium", previewMode === "desktop" ? "bg-brand-navy text-white" : "bg-white text-slate-600")}
            >
              Desktop
            </button>
            <button
              ...
              className={cn("px-3 py-1.5 text-xs font-medium", previewMode === "mobile" ? "bg-brand-navy text-white" : "bg-white text-slate-600")}
            >
              Mobile
            </button>
          </div>
        ...
        <div className="overflow-hidden rounded-xl border border-slate-200">
          ...
          <div className={cn("overflow-x-auto bg-white transition-all", previewMode === "mobile" ? "max-w-[380px]" : "w-full")}>
            <div className={cn("flex items-center justify-between gap-4 px-4", shadowClass)} style={{ height: heightPx, backgroundColor: form.bgColor, color: form.textColor }}>
              <span className="shrink-0 whitespace-nowrap text-sm font-bold" style={{ color: form.iconTextColor }}>
                {form.logoTextTh || "Millimed"}
              </span>
              <nav className={cn("flex items-center gap-1", form.menuWrap === "wrap" ? "flex-wrap justify-end" : "flex-nowrap overflow-x-auto")}>
                {navLinks.map((link, i) => {
                  ...
                  return (
                    <button
                      ...
                      style={active ? { backgroundColor: form.activeBgColor, color: form.activeTextColor } : { color: form.textColor }}
                      onMouseEnter={(e) => {
                        if (active) return;
            ...
            <div className="flex min-h-[60px] items-center justify-center text-xs text-slate-300">พื้นที่เนื้อหาหน้าเว็บ</div>
          </div>
        </div>
      </div>
