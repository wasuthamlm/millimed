    ...
    <div className="flex flex-col gap-4 rounded-2xl border border-slate-100 bg-white p-6 shadow-sm">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div>
          ...
          <select className={selectClass} value={form.transitionEffect} onChange={(e) => update("transitionEffect", e.target.value)}>
        ...
        <div>
          ...
          <select className={selectClass} value={form.direction} onChange={(e) => update("direction", e.target.value)} disabled={form.transitionEffect !== "slide"}>
        ...
        <div>
          ...
          <input type="range" min={200} max={1500} step={100} value={form.transitionSpeedMs} onChange={(e) => update("transitionSpeedMs", Number(e.target.value))} className="w-full accent-brand-navy" />
        </div>
        <div>
          <label className={labelClass}>เวลาแสดงต่อภาพ ({(form.displayDurationMs / 1000).toFixed(1)} วิ)</label>
          <input type="range" min={2000} max={10000} step={500} value={form.displayDurationMs} onChange={(e) => update("displayDurationMs", Number(e.target.value))} className="w-full accent-brand-navy" />
        </div>
      </div>

      <div className="grid gap-3 sm:grid-cols-2">
        {toggles.map((t) => (
          <div key={t.key} className="flex items-center justify-between gap-3 rounded-lg border border-slate-100 bg-slate-50 px-4 py-3">
            <span className="text-sm font-medium text-slate-700">{t.label}</span>
            <Toggle checked={Boolean(form[t.key])} onChange={(v) => update(t.key, v as never)} label={t.label} />
          </div>
        ))}
      </div>
