...
export function FooterManager({
  ...
  const updateLink = (colId: string, linkId: string, patch: { label?: string; href?: string }) => {
    setColumns((prev) => prev.map((c) => (c.id === colId ? { ...c, links: c.links.map((l) => (l.id === linkId ? { ...l, ...patch } : l)) } : c)));
  };
  ...
  const addLink = (colId: string) => {
    setColumns((prev) => prev.map((c) => (c.id === colId ? { ...c, links: [...c.links, { id: `l${Date.now()}`, label: "ลิงก์ใหม่", href: "/" }] } : c)));
  };
    ...
    <div className="flex flex-col gap-6">
      <div className="flex flex-col gap-4 rounded-2xl border border-slate-100 bg-white p-6 shadow-sm">
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          ...
          <ColorField label="Accent / Hover / Active" value={theme.accentColor} onChange={(v) => updateTheme("accentColor", v)} />
        </div>
      </div>
          ...
          {[1, 2, 3, 4].map((n) => (
            <button
              ...
              className={cn(
                ...
                theme.desktopColumns === n ? "border-brand-navy bg-brand-navy/5 text-brand-navy" : "border-slate-200 text-slate-600 hover:bg-slate-50"
              )}
            >
              {n} คอลัมน์{n === 3 && " (แนะนำ)"}
            </button>
      ...
      <div className="rounded-2xl border border-slate-100 bg-white shadow-sm">
        <div className="flex items-center gap-1 overflow-x-auto border-b border-slate-100 px-4">
          {visibleColumns.map((col, i) => (
            <button
              ...
              className={cn(
                ...
                activeTab === i ? "border-brand-navy text-brand-navy" : "border-transparent text-slate-500 hover:text-slate-700"
              )}
            >
          ...
          {visibleColumns.length < theme.desktopColumns && (
            <button type="button" onClick={addColumn} className="inline-flex shrink-0 items-center gap-1 px-4 py-3 text-sm font-medium text-brand-navy hover:text-brand-navy-dark">
          ...
          {visibleColumns[activeTab] ? (
            <div className="flex flex-col gap-4">
              <div className="flex items-center gap-2">
                ...
                />
                <button type="button" onClick={() => removeColumn(visibleColumns[activeTab].id)} className="rounded-md p-2 text-red-400 hover:bg-red-50 hover:text-red-600" aria-label="ลบคอลัมน์">
                ...
                {visibleColumns[activeTab].links.map((link) => (
                  <div key={link.id} className="flex items-center gap-2">
                    <input
                      type="text"
                      value={link.label}
                      onChange={(e) => updateLink(visibleColumns[activeTab].id, link.id, { label: e.target.value })}
                      className="w-40 rounded-lg border border-slate-200 px-2.5 py-1.5 text-sm outline-none focus:border-brand-navy"
                    />
                    <input
                      type="text"
                      value={link.href}
                      onChange={(e) => updateLink(visibleColumns[activeTab].id, link.id, { href: e.target.value })}
                    ...
                    />
                    <button type="button" aria-label="ลบลิงก์" onClick={() => removeLink(visibleColumns[activeTab].id, link.id)} className="rounded-md p-1.5 text-red-400 hover:bg-red-50 hover:text-red-600">
                      <TrashIcon className="h-4 w-4" />
                    </button>
                  </div>
                ))}
                {visibleColumns[activeTab].links.length === 0 && (
                  <p className="rounded-lg border border-dashed border-slate-200 py-6 text-center text-sm text-slate-400">ไม่มีลิงก์ในคอลัมน์นี้ กดปุ่มด้านล่างเพื่อเพิ่มลิงก์แรก</p>
                )}
              </div>

              <button type="button" onClick={() => addLink(visibleColumns[activeTab].id)} className="inline-flex w-fit items-center gap-1.5 rounded-lg border border-dashed border-slate-300 px-3 py-2 text-sm font-medium text-brand-navy hover:bg-slate-50">
                <PlusIcon className="h-4 w-4" />
                เพิ่มลิงก์
              </button>
            </div>
            ...
            <div className="flex flex-col items-center gap-3 py-8 text-center">
              ...
              <button type="button" onClick={addColumn} className="inline-flex items-center gap-1.5 rounded-lg bg-brand-navy px-3 py-2 text-sm font-medium text-white hover:bg-brand-navy-dark">
                <PlusIcon className="h-4 w-4" />
                เพิ่มคอลัมน์
              </button>
            </div>
      ...
      <div className="rounded-2xl border border-slate-100 bg-white p-6 shadow-sm">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div>
            ...
            <input type="text" value={contact.tagline} onChange={(e) => setContact({ ...contact, tagline: e.target.value })} className={inputClass} />
          </div>
          <div>
            <label className="mb-1.5 block text-xs font-medium text-slate-500">เบอร์โทร</label>
            <input type="text" value={contact.phone} onChange={(e) => setContact({ ...contact, phone: e.target.value })} className={inputClass} />
          </div>
          <div>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div>
            ...
            <input
              ...
              placeholder={`© ${new Date().getFullYear()} Millimed สงวนลิขสิทธิ์`}
          ...
          <div>
            ...
            <input
              ...
              placeholder={`© ${new Date().getFullYear()} Millimed. All rights reserved.`}
              className={inputClass}
            />
          </div>
        </div>
      ...
      <div className="overflow-hidden rounded-2xl" style={{ backgroundColor: theme.bgColor }}>
        <div className="flex items-center justify-between px-6 py-4">
          ...
          <button type="button" onClick={() => setPreviewLang((l) => (l === "th" ? "en" : "th"))} className="rounded-full px-3 py-1 text-xs font-medium" style={{ backgroundColor: theme.accentColor, color: theme.bgColor }}>
            ภาษาไทย ({previewLang.toUpperCase()})
          </button>
        </div>
        <div className="grid gap-6 px-6 pb-6" style={{ gridTemplateColumns: `repeat(${Math.max(theme.desktopColumns, 1)}, minmax(0, 1fr))` }}>
        ...
        <div className="border-t px-6 py-3 text-center text-xs opacity-70" style={{ borderColor: theme.accentColor + "33", color: theme.textColor }}>
          {previewLang === "th" ? theme.copyrightTh || `© ${new Date().getFullYear()} Millimed สงวนลิขสิทธิ์` : theme.copyrightEn || `© ${new Date().getFullYear()} Millimed. All rights reserved.`}
        </div>
      </div>
