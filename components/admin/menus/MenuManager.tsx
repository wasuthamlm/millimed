...
import { createNavLink, updateNavLink, deleteNavLink, toggleNavLink, reorderNavLinks } from "@/app/admin/menus/actions";
...
export function MenuManager({ initialTree }: { initialTree: MenuNode[] }) {
  ...
  const submitEdit = (id: string) => {
    ...
    startTransition(async () => {
      ...
      updateTree((nodes) => mapNode(nodes, id, (n) => ({ ...n, labelTh: editForm.labelTh, labelEn: editForm.labelEn || null, href: editForm.href })));
      setEditingId(null);
    });
  };
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between gap-4">
        <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-medium text-slate-500">{levels} ระดับ</span>
        <button
          type="button"
      ...
      {addingParentId === "root" && <AddForm form={addForm} setForm={setAddForm} onSubmit={submitAdd} onCancel={() => setAddingParentId(null)} pending={pending} />}
  ...
  return (
    <div className="flex flex-col gap-3 rounded-2xl border border-slate-100 bg-white p-4 shadow-sm sm:flex-row sm:items-end sm:flex-wrap">
      ...
      <div className="flex gap-2">
        <button type="button" disabled={pending} onClick={onSubmit} className="rounded-lg bg-brand-navy px-4 py-2 text-sm font-medium text-white hover:bg-brand-navy-dark disabled:opacity-60">
          บันทึก
        </button>
          ...
          <>
            <td className="px-4 py-3.5" style={{ paddingLeft: 16 + depth * 24 }}>
              <div className="flex items-center gap-2">
                ...
                {hasChildren && <span className="rounded-full bg-slate-100 px-2 py-0.5 text-xs text-slate-500">{node.children.length}</span>}
              </div>
            </td>
            ...
            <td className="px-6 py-3.5">
              <div className="flex items-center justify-end gap-1">
                <button type="button" onClick={() => onOpenAdd(node.id)} aria-label="เพิ่มเมนูย่อย" title="เพิ่มเมนูย่อย" className="rounded-md p-1.5 text-slate-500 hover:bg-slate-100 hover:text-brand-navy">
                  <PlusIcon className="h-4 w-4" />
                </button>
                <button type="button" onClick={() => onStartEdit(node)} aria-label="แก้ไข" className="rounded-md p-1.5 text-slate-500 hover:bg-slate-100 hover:text-brand-navy">
                  <PencilIcon className="h-4 w-4" />
                </button>
                <button type="button" onClick={() => onRemove(node.id)} aria-label="ลบ" className="rounded-md p-1.5 text-red-500 hover:bg-red-50">
