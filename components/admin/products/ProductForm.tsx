...
import { createProduct, deleteProduct, updateProduct, type ProductFormInput } from "@/app/admin/products/actions";
...
export function ProductForm({
  ...
  const seoResult = useMemo(
    ...
    [form.nameTh, form.nameEn, form.seoTitle, form.seoTitleEn, form.seoDesc, form.seoDescEn, form.descriptionTh, form.sku, form.imageUrl]
  );
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-1 gap-4 rounded-2xl border border-slate-100 bg-white p-6 shadow-sm sm:grid-cols-2">
        ...
        <div>
          ...
          <input className={inputClass} value={form.nameTh} onChange={(e) => update("nameTh", e.target.value)} />
        </div>
        ...
        <div className="sm:col-span-2">
          <ImageUploader label="รูปภาพสินค้า" value={form.imageUrl} onChange={(url) => update("imageUrl", url)} />
        </div>
      ...
      <div className="grid grid-cols-1 gap-4 rounded-2xl border border-slate-100 bg-white p-6 shadow-sm sm:grid-cols-2">
        ...
        <div className="sm:col-span-2 border-t border-slate-100 pt-4">
          ...
          <p className="truncate text-sm text-slate-600">{`${SITE_URL}/products/${form.id || "…"}`}</p>
        </div>
      </div>
