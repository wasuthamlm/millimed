...
export function HeroBanners({ banners }: { banners: HeroBannerItem[] }) {
  ...
  const slide = (
    <div className="relative aspect-[21/9] w-full overflow-hidden sm:aspect-[3/1]">
      <Image src={current.image} alt={current.titleTh} fill priority sizes="100vw" className="object-cover" />
    </div>
  );
        ...
        <div className="absolute bottom-4 left-1/2 flex -translate-x-1/2 gap-2">
          {banners.map((banner, i) => (
            <button
              ...
              onClick={() => setIndex(i)}
              className={cn("h-2 w-2 rounded-full transition-all", i === index ? "w-6 bg-white" : "bg-white/50")}
            />
          ))}
        </div>
      )}
